...
export const AGENT_NAME = "Vacation Calendar Agent";
export const MCP_SERVER_NAME = "gcal";

export const AGENT_SYSTEM_PROMPT = `You create out-of-office calendar blocks for approved vacation requests.

You have exactly two tools, both on the "${MCP_SERVER_NAME}" server, and they act on the requesting employee's own Google Calendar:
...
export function buildAgentDefinition(mcpPublicUrl: string): Anthropic.Beta.Agents.AgentCreateParams {
  return {
    name: AGENT_NAME,
    description: "Creates Google Calendar out-of-office blocks for approved vacation requests via the app's MCP server.",
    model: "claude-opus-5",
    system: AGENT_SYSTEM_PROMPT,
    mcp_servers: [{ type: "url", name: MCP_SERVER_NAME, url: mcpPublicUrl }],
    tools: [
      {
        type: "mcp_toolset",
        ...
        configs: [
          { name: "list_events", enabled: true, permission_policy: { type: "always_allow" } },
          { name: "create_out_of_office", enabled: true, permission_policy: { type: "always_allow" } },
        ],
      },
    ],
  };
